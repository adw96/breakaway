...
Wow! That changed our original estimate of `r round(estimated_richness$estimate)` significantly -- now we think there are `r round(estimated_richness_10$estimate)` species level genome bins! What's happening?


*Note: At time of writing (August 2019), `breakaway` produced an estimate of 7724. If this number has changed significantly since then, it's because we are constantly making improvements to the code base, which may change estimates -- usually by a little but sometimes by more. Please let me know or [log an issue](https://github.com/adw96/breakaway/issues) to let us know if this occured!*
